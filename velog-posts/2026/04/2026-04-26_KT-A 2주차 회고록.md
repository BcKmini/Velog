...
## 들어가며
ML에 대해 2일 동안 DL에 대해 1일 학습을 진행했다. 기간이 짧은 기간 동안 중요한내용을 빠르게 학습하고 바로 실습(코드)으로 진행해 개인이 문제를 해결하기보다 라이브러리 및 함수에 대해 익숙해지고 학습한 모델이 어떤구조와 어떤 흐름으로 이어가는지에 대한 이해를 배운 거 같다. 
...
![](https://velog.velcdn.com/images/mi_nini/post/c41caf23-27cd-4497-96ff-198b1e2a0ba0/image.png)
...
| 평가 지표 | 계산식 |
| :--- | :--- |
| **MAE**<br/>(평균 절대 오차) | $MAE = \frac{1}{n} \sum \vert y_i - \hat{y}_i \vert$ |
| **MSE**<br/>(평균 제곱 오차) | $MSE = \frac{1}{n} \sum (y_i - \hat{y}_i)^2$ |
| **RMSE**<br/>(평균 제곱근 오차) | $RMSE = \sqrt{\frac{1}{n} \sum (y_i - \hat{y}_i)^2}$ |
| **MAPE**<br/>(평균 절대 백분율 오차) | $MAPE = \frac{100\%}{n} \sum \left\vert \frac{y_i - \hat{y}_i}{y_i} \right\vert$ |
| **MPE**<br/>(평균 백분율 오차) | $MPE = \frac{100\%}{n} \sum \left( \frac{y_i - \hat{y}_i}{y_i} \right)$ |
[회귀 평가지표]
...
![](https://velog.velcdn.com/images/mi_nini/post/e83445c0-5e44-4893-a050-9f663dfec9a5/image.png)
&gt; 출처/참고 : https://glassboxmedicine.com/2019/02/17/measuring-performance-the-confusion-matrix/

| 평가 지표 | 설명 | 계산식 |
| :--- | :--- | :--- |
| **정확도**<br/>(Accuracy) | 실제 분류를 정확하게 예측한 비율 | $Accuracy = \frac{TP + TN}{TP + TN + FP + FN}$ |
| **정밀도**<br/>(Precision) | Positive로 예측한 것 중 실제 Positive인 비율 | $Precision = \frac{TP}{TP + FP}$ |
| **민감도/재현율**<br/>(Recall/Sensitivity) | 실제 Positive(P) 중 Positive로 예측한 비율 | $Recall = \frac{TP}{TP + FN}$ |
| **F1-score** | 정밀도와 재현율의 조화평균 | $F1 = 2 \times \frac{Precision \times Recall}{Precision + Recall}$ |
...
![](https://velog.velcdn.com/images/mi_nini/post/e94fff01-1cc7-46b2-bd41-12a87b829f40/image.png)
...
![](https://velog.velcdn.com/images/mi_nini/post/6602fb57-6196-418c-8560-26e8035ce5d2/image.png)
* Grid Search (그리드 서치): 지정한 파라미터 값들의 '모든 경우의 수'를 다 조합해서 학습해 보고 가장 최고 성능(best_score_)을 내는 조합(best_params_)을 찾는 방법. 탐색 공간이 넓어지면 시간이 늘어난다.
...
&gt; 출처/참고 https://starcell.github.io/ai/dl-basic/
![](https://velog.velcdn.com/images/mi_nini/post/d610449a-1385-423d-a7e4-b992b95de981/image.png)
...
딥러닝은 층과 층 사이에 '활성화 함수'를 넣어 비선형성을 추가한다. 이것이 없으면 아무리 층을 깊게 쌓아도 결국 단순한 선형 회귀 모형과 다를 바가 없어진다. 은닉층에서는 주로 'relu'를 사용하여 딥러닝의 고질적 문제인 [기울기 소실(Vanishing Gradient)](https://eureka7.tistory.com/20)을 방지한다.
...
---
## 생각정리
대학원 면접 보러 갔을 당시 내가 지정했던 주제인 Why Multi-class Classification Needs Softmax가 주마등처럼 스쳐가는 수업이었던 거 같다. 수업내용에서는 Multi-class를 크게 다루지 않았지만 생각이 딱 들었고 학부 연구생으로써 연구실에 처음 들어갔을 때 읽었던 논문들이 생각나게 되는 수업이었던 거 같다.
