...
---|---  
**MAE**  
(평균 절대 오차)| MAE=1n∑∣yi−y^i∣MAE = \frac{1}{n} \sum \vert y_i - \hat{y}_i \vertMAE=n1​∑∣yi​−y^​i​∣  
**MSE**  
(평균 제곱 오차)| MSE=1n∑(yi−y^i)2MSE = \frac{1}{n} \sum (y_i - \hat{y}_i)^2MSE=n1​∑(yi​−y^​i​)2  
**RMSE**  
(평균 제곱근 오차)| RMSE=1n∑(yi−y^i)2RMSE = \sqrt{\frac{1}{n} \sum (y_i - \hat{y}_i)^2}RMSE=n1​∑(yi​−y^​i​)2​  
**MAPE**  
(평균 절대 백분율 오차)| MAPE=100%n∑∣yi−y^iyi∣MAPE = \frac{100\%}{n} \sum \left\vert \frac{y_i - \hat{y}_i}{y_i} \right\vertMAPE=n100%​∑∣∣∣∣​yi​yi​−y^​i​​∣∣∣∣​  
**MPE**  
(평균 백분율 오차)| MPE=100%n∑(yi−y^iyi)MPE = \frac{100\%}{n} \sum \left( \frac{y_i - \hat{y}_i}{y_i} \right)MPE=n100%​∑(yi​yi​−y^​i​​)  
...
---|---|---  
**정확도**  
(Accuracy)| 실제 분류를 정확하게 예측한 비율| Accuracy=TP+TNTP+TN+FP+FNAccuracy = \frac{TP + TN}{TP + TN + FP + FN}Accuracy=TP+TN+FP+FNTP+TN​  
**정밀도**  
(Precision)| Positive로 예측한 것 중 실제 Positive인 비율| Precision=TPTP+FPPrecision = \frac{TP}{TP + FP}Precision=TP+FPTP​  
**민감도/재현율**  
(Recall/Sensitivity)| 실제 Positive(P) 중 Positive로 예측한 비율| Recall=TPTP+FNRecall = \frac{TP}{TP + FN}Recall=TP+FNTP​  
**F1-score**|  정밀도와 재현율의 조화평균| F1=2×Precision×RecallPrecision+RecallF1 = 2 \times \frac{Precision \times Recall}{Precision + Recall}F1=2×Precision+RecallPrecision×Recall​  
...
![](https://velog.velcdn.com/images/mi_nini/post/6602fb57-6196-418c-8560-26e8035ce5d2/image.png)
  ...
  * Grid Search (그리드 서치): 지정한 파라미터 값들의 '모든 경우의 수'를 다 조합해서 학습해 보고 가장 최고 성능(best _score_)을 내는 조합(best _params_)을 찾는 방법. 탐색 공간이 넓어지면 시간이 늘어난다.
