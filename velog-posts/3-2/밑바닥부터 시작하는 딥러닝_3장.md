...
<pre><code class="language-py">x = np.arrge(-5.0, 5.0, 0.1)
y = sigmit(x)
plt.plot(x, y)
...
<pre><code class="language-py">C = np.array([[1,2], [3,4]])
C.shpae
# (2,2)
A = np.array([[1,2,3], [4,5,6]])
A.shape
...
(x_trian, t_train), (x_test, t_test) = \ 
    load_mnist(flatten=True, normalize=False)
# flatten = True로 설정해 읽어 들인 이미지는 1차원 넘파이 배열로 저장 되어 있다.
